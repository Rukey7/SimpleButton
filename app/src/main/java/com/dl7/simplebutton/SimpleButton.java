...
public class SimpleButton extends View {
    ...
    private void _init(Context context, AttributeSet attrs) {
        ...
        mHorizontalPadding = (int) MeasureUtils.dp2px(context, 10f);
    ...
    private int _adjustText(int maxWidth) {
        ...
        if (mIsChecked && !TextUtils.isEmpty(mTextChecked)) {
            if (mFontLenChecked + allPadding > maxWidth) {
                ...
                mShowText = MeasureUtils.clipShowText(mTextChecked, mPaint, maxTextWidth);
            ...
            // 计算能显示的字体长度
            float maxTextWidth = maxWidth - allPadding - pointWidth * 3;
            mShowText = MeasureUtils.clipShowText(mText, mPaint, maxTextWidth);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 如果你用了精确测量则 mHorizontalPadding 和 mVerticalPadding 会无效
    ...
    public void setChecked(boolean checked) {
        ...
        mIsChecked = checked;
        // 注意，这里用 requestLayout() 而不是 invalidate()，因为 invalidate() 没法回调 onMeasure() 进行测量，
        // 如果控件自适应大小的话是有可能改变大小的，所以直接用 requestLayout()
        requestLayout();
//        invalidate();
    ...
    /**
     * 如果调用这些接口进行属性设置的话最后调用一下这个接口，以刷新界面，建议属性直接在布局里设置
     */
    public void update() {
        requestLayout();
    }
